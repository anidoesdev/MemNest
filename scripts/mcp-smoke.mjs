...
if (tools !== 'dashboard_forget,dashboard_read,forget,history,ingest,profile,recall,remember,show_dashboard') {
...
// The dashboard app's HTML is bundled from the private @memnest/mcp-app: the published package must carry it.
send({ jsonrpc: '2.0', id: 5, method: 'resources/read', params: { uri: 'ui://memnest/dashboard' } });
const dashboardApp = (await reply(5)).result?.contents?.[0];
if (dashboardApp?.mimeType !== 'text/html;profile=mcp-app' || !/^<!doctype html>/i.test(dashboardApp.text ?? '')) {
  throw new Error(`MCP smoke: dashboard app ${JSON.stringify(dashboardApp)?.slice(0, 200)}`);
}
